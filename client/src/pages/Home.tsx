  ...
  return (
    <div className="min-h-screen bg-[#f7f6f1] text-[#173b2d]">
      <div className="hidden bg-[#f29a3d] px-4 py-2 text-center text-xs font-bold tracking-[0.04em] text-[#2d1c0b] sm:block">
        <span>New here? Get NPR 150 off your first order with code </span><span className="rounded-full bg-[#173b2d] px-2 py-1 text-[#fff8eb]">NAMASTE150</span>
      </div>
      ...
      <header className="sticky top-0 z-30 border-b border-[#e1e5de]/80 bg-[#f7f6f1]/95 backdrop-blur-xl">
        <div className="container flex h-[72px] items-center gap-3 lg:gap-7">
          ...
          <button onClick={() => setLocationOpen(true)} className="hidden shrink-0 items-center gap-2 rounded-xl px-2 py-2 text-left transition-colors hover:bg-[#edf0e9] md:flex" aria-label="Choose delivery location">
            ...
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#76837b]">Deliver to</div>
          ...
          <div className="relative min-w-0 flex-1">
            ...
            <input className="h-11 w-full rounded-2xl border border-[#dfe5dd] bg-white pl-11 pr-4 text-sm font-medium outline-none transition focus:border-[#f29a3d] focus:ring-4 focus:ring-[#f29a3d]/10" placeholder="Search for products, brands and more" onKeyDown={event => event.key === "Enter" && toast.info("Search is ready to connect to your full catalog.")} />
          </div>

          <div className="hidden items-center gap-1 xl:flex">
            <button className="flex items-center gap-2 rounded-xl px-3 py-2 text-left hover:bg-[#edf0e9]" onClick={() => { setLoginStep("phone"); setLoginOpen(true); }}>
              <UserRound className="h-[19px] w-[19px]" />
              <div><div className="text-[10px] text-[#76837b]">Hello, {loggedIn ? "Riya" : "there"}</div><div className="flex items-center gap-1 text-xs font-bold">Account <ChevronDown className="h-3 w-3" /></div></div>
            </button>
          ...
          <button className="grid h-10 w-10 place-items-center rounded-xl hover:bg-[#edf0e9] xl:hidden" onClick={() => setMobileMenuOpen(current => !current)} aria-label="Open menu"><Menu className="h-5 w-5" /></button>
        </div>
        {mobileMenuOpen && <div className="container flex items-center gap-3 border-t border-[#e1e5de] py-3 xl:hidden"><button onClick={() => setLocationOpen(true)} className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 text-xs font-bold"><MapPin className="h-4 w-4 text-[#f29a3d]" />{selectedLocation}</button><button onClick={() => setLoginOpen(true)} className="rounded-xl bg-[#12372a] px-3 py-2 text-xs font-bold text-white">{loggedIn ? "My account" : "Sign in"}</button><button onClick={() => setTrackOpen(true)} className="rounded-xl bg-white px-3 py-2 text-xs font-bold">Track order</button><button onClick={() => setCartOpen(true)} className="rounded-xl bg-white px-3 py-2 text-xs font-bold">Bag ({cartCount})</button></div>}
      </header>
      ...
      <main>
        <section className="container pt-4 sm:pt-6">
          <div className="hero-grid relative isolate min-h-[420px] overflow-hidden rounded-[30px] p-6 text-[#fff8eb] sm:p-10 lg:min-h-[454px] lg:p-14">
            ...
            <div className="relative z-10 max-w-[570px]">
              <div className="mb-6 flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-[0.14em] text-[#cde4d7]"><span className="rounded-full bg-[#f29a3d] px-3 py-1.5 text-[#2d1c0b]">The Nepal edit</span><span>·</span><span>New season, new finds</span></div>
              <h1 className="font-display max-w-[560px] text-[42px] font-extrabold leading-[1.04] tracking-[-0.06em] sm:text-[57px]">Everything you need,<br /><span className="text-[#f5ac60]">delivered your way.</span></h1>
              <p className="mt-5 max-w-[445px] text-[15px] leading-7 text-[#d2e3d8] sm:text-base">Discover good things from trusted sellers across Nepal — from your morning groceries to the tech you’ve been waiting for.</p>
              <div className="mt-8 flex flex-wrap gap-3"><button onClick={() => document.getElementById("shop")?.scrollIntoView({ behavior: "smooth" })} className="group inline-flex items-center gap-3 rounded-xl bg-[#f29a3d] px-5 py-3.5 text-sm font-extrabold text-[#2d1c0b] shadow-[0_9px_0_#c96f20] hover:bg-[#ffad59]"><span>Start exploring</span><ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></button><button onClick={() => setLocationOpen(true)} className="inline-flex items-center gap-2 rounded-xl border border-white/20 bg-white/10 px-5 py-3.5 text-sm font-bold text-white hover:bg-white/15"><LocateFixed className="h-4 w-4 text-[#f5ac60]" />Set delivery location</button></div>
        ...
        <section className="container py-8 sm:py-10">
          <div className="mb-4 flex items-end justify-between"><div><p className="mb-1 text-[11px] font-extrabold uppercase tracking-[0.16em] text-[#d67b2c]">Browse the good stuff</p><h2 className="font-display text-2xl font-extrabold tracking-[-0.04em] sm:text-3xl">Shop by category</h2></div><button className="hidden items-center gap-1 text-xs font-extrabold text-[#c46d24] sm:flex" onClick={() => { setSelectedCategory("All"); document.getElementById("shop")?.scrollIntoView({ behavior: "smooth" }); }}>View all <ArrowRight className="h-3.5 w-3.5" /></button></div>
        ...
        <section id="shop" className="border-y border-[#e1e5de] bg-[#fbfaf7] py-10 sm:py-12">
          <div className="container">
            <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="mb-1 text-[11px] font-extrabold uppercase tracking-[0.16em] text-[#d67b2c]">Curated for your day</p><h2 className="font-display text-2xl font-extrabold tracking-[-0.04em] sm:text-3xl">Popular near you</h2><p className="mt-2 text-sm text-[#718076]">Handpicked by our local team · delivery to {selectedLocation.split(",")[0]}</p></div><div className="flex items-center gap-2"><button className="grid h-9 w-9 place-items-center rounded-full border border-[#e1e5de] bg-white hover:border-[#f29a3d]"><ChevronLeft className="h-4 w-4" /></button><button className="grid h-9 w-9 place-items-center rounded-full border border-[#e1e5de] bg-white hover:border-[#f29a3d]"><ChevronRight className="h-4 w-4" /></button></div></div>
            <div className="hide-scrollbar mt-6 flex gap-2 overflow-x-auto pb-2"><button onClick={() => setSelectedCategory("All")} className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-extrabold ${selectedCategory === "All" ? "bg-[#12372a] text-white" : "border border-[#dfe5dd] bg-white text-[#5d6f64]"}`}>All finds</button>{categories.map(category => <button key={category.label} onClick={() => setSelectedCategory(category.label)} className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-extrabold ${selectedCategory === category.label ? "bg-[#12372a] text-white" : "border border-[#dfe5dd] bg-white text-[#5d6f64]"}`}>{category.label}</button>)}</div>
